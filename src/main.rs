        ...
        match arg.as_str() {
            "rom" => {
                ...
                for n in 0..(addr_lines) {
                    inputs.push(
                        {
                            let button = sls::Node::new(NodeType::TOGGLE_BUTTON, Some(n.to_string()), id.to_string()).at(-100.0, n as f32 * 100.0);
                            id+=1;
                            button
                        }
                    )
                ...
                for n in 0..BITWIDTH {
                    outputs.push(
                        {
                            let light = sls::Node::new(NodeType::LIGHT_BULB, Some(n.to_string()), id.to_string()).at(100.0, n as f32 * 100.0);
                            id+=1;
                            light
                        }
                    )
                ...
                for bit in 0..BITWIDTH {
                    let mut layer_i = if muxes[bit].len()>0 {muxes[bit].len()-1} else {0};
                    while layer_i>0 {
                        let iter = muxes[bit][layer_i].iter().map(|n|(n.get_size().unwrap(),n.get_id())).enumerate();
                    ...
                    if let Some(first_muxes) = muxes[bit].first() {
                        ...
                        for mux in first_muxes {
                            for mux_bit in 0..first_size {
                                let byte: u8 = if total_byte<buf.len() {buf[total_byte]} else {0};
                                if byte & (1<<bit) !=0 {
