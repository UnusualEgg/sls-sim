...
struct Node {
    ...
    outputs: Rc<RefCell<Vec<bool>>>,
    #[serde(skip)]
    next_outputs: Vec<bool>,
    #[serde(skip, default)]
    flip_flop: bool,
    id: ID,
    x: f32,
...
fn main() {
    ...
    let mut n: Circuit = serde_json::from_reader(std::io::BufReader::new(f)).unwrap();
